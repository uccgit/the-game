...
def continuegame():
    """Prompt to choose saved game and load player and world_map states"""
    global world_map
    global player
...
def homescreen():
    ...
    choice = ''
    while True:
        current_zone = world_map.loadZone()
        os.system("cls" if os.name == "nt" else "clear")
        choice = current_zone.get_options()
        if choice == 'e':
            os.system("cls" if os.name == "nt" else "clear")
            explore(current_zone)
        elif choice == 'c':
            os.system("cls" if os.name == "nt" else "clear")
            player.get_character_sheet_full()
            raw_input("Press \"Enter\" to continue...")
        elif choice == 'r':
            cost = player.level * 10 + 15
            print "Cost: {}g".format(cost)
            if raw_input("Accept? (Y/N): ").lower() == 'y':
                if player.take_gold(cost) is True:
                    player.give_health(player.get_max_health())
            ...
            os.system("cls" if os.name == "nt" else "clear")
            tmp_item = world_map.current_zone.market.get_inventory(player)
            if tmp_item is not None:
                player.give_item(tmp_item)
            raw_input("Press \"Enter\" to continue...")
        elif choice == 't':
            os.system("cls" if os.name == "nt" else "clear")
            world_map.printMap()
            if world_map.current_zone.z_type == "wild":
                explore(world_map.current_zone)
            else:
                print "You make your way to " + world_map.current_zone.z_name
                raw_input("Press \"Enter\" to continue...")
        elif choice == 'q':
            os.system("cls" if os.name == "nt" else "clear")
            sys.exit()
...
def saveGame():
    ...
    cPickle.dump(world_map, f, protocol=0)
    f.close()
    print "...Done!"
    raw_input("Press \"Enter\" to continue...")
