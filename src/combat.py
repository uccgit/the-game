...
def getAction(a, b):
    ...
    print b.name
    print "Health: " + repr(b.health) + "/" + repr(b.get_max_health())
    print "P: {:2} P: {:2} T: {:2} V: {:2}".format(
           b.stat_list[0], b.stat_list[1], b.stat_list[2], b.stat_list[3])
    print "----------------------VERSUS"
    print a.name
    print "Health: " + repr(a.health) + "/" + repr(a.get_max_health())
    print "P: {:2} P: {:2} T: {:2} V: {:2}".format(
    ...
    while True:
        ...
        if c_state == "standby":
            ...
            if loot[1] is not None:
                a.give_item(loot[1])
            a.give_gold(loot[0])
            a.give_exp((a.exp_needed / (9 + a.level)) * (b.level / a.level))
            raw_input("Press \"Enter\" to continue...")
            break

        elif c_state == "inventory":
            a.get_inventory("combat")
            n_state = "standby"

        elif c_state == "info":
            b.get_character_sheet_full()
            raw_input("Press \"Enter\" to continue...")
            n_state = "standby"
